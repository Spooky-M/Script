...
def main(argv):
    ...
    project_path = str(args[0])     # set project_path

    # set all the necessary variables (c_tool, p_tool, package, aux_classpath...)
    for opt, arg in opts:
        if opt == TOOLS_ARG:
            tools = arg.split(",")
            for a in tools:
    ...
    if os.path.isfile(project_path):
        ...
        # edit project_path, set project_path_appsrc, project_path_package and directory
    ...
    for t in tools:
        if t in (SPOTBUGS_4_0_0_BETA4, SPOTBUGS_4_0_0_BETA3, SPOTBUGS_4_0_0_BETA2, SPOTBUGS_4_0_0_BETA1,
            ...
            # walk the file tree and run checkstyle for every .java file
            pattern = "*.java"
            stop: bool = False
...
def error_message(message: str, exit_code=1):
    ...
    :param message: success message to print
    :param exit_code: exit code for sys.exit() function
    :return:
    """
    print("\n" + message)
    sys.exit(exit_code)
